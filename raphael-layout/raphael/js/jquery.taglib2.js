        ...
        getStimeText(n){
            ...
            if(n.upState == "3"){
                nodeTime = n.ruleNodeType == "Start"? "" :
                    n.ruleNodeType == "End"?"总耗时  " + n.upTime : "耗时  " + n.upTime;
        ...
        tagObj = {
            init: function (options) {
                ...
                var tagOperation = {
                    ...
                    //从数据库中获取保存的数据
                    getServerData: function (url, params ,update) {
                        var h = this;
                        var param = [];
                        if (params) {
                        ...
                        var ajaxOptions = {
                            ...
                            success: function (data1) {
                                $("#"+canvasid + " .loading-table").empty();
                                if(data1 == [] && data1.length < 0 && _setting.g.model == "state"){
                                    g.nodataTips();
                                    return;
                                }
                                if(update == "create"){
                                ...
                                for (var i = 0; i < data.length; i++) {
                                    if (data[i].nodeId !== null && data[i].nodeId !== undefined) {
                                        ...
                                        if(_setting.g.model == "state"){
                                            scan.upState = String(data[i].nodeStatus);
                    ...
                    //跟新应用状态图
                    updateState: function(update,raphaelData){
                        tagOperation.createNodeLine(raphaelData, update);
                    },
                    //初始化和跟新画布元素
                    createNodeLine: function(raphaelData, update){
                        var h = this;
                        var data = raphaelData;
                        $("#"+canvasid + " .loading-table").empty();
                        if(data == [] && data.length < 0 && _setting.g.model == "state"){
                            g.nodataTips();
                            return;
                        }
                        if(update == "create"){
                ...
                if(g.newProcess == true || g.newProcess == "true"){
                    ...
                    tagOperation.createNodeLine(g.urlData, "create");
                }
