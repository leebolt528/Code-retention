$(function(){
    ...
    function getFitLogsFormat(logs){
        ...
        addLogs.map(function(log,index){
            addLogsText += "<div class='bcloud-item-log'>"+
                ...
                "<div class='item-content'>"+log.message+"</div>"+
                "</div>"+
                "<hr>";
        });
        return addLogsText;
    }
    function raphaelState(componentsImgMap){
        tag = $("#state_canvas").raphael().init({
            cwidth: 1900, //画布宽度
            cheight: 1500, //画布高度,
            model:"state",
            url: $webpath + '/servflow/findNode/appId/' + urlParams.applicationId,  //url 获取该画布流程图信息
            stateUrl:$webpath + '/servflow/v1/api/ruleChain/ruleState/' + urlParams.applicationId,
            newProcess:false,
    ...
    function drawPages(boolean){
        ...
        $.ajax({
            ...
            dataType: "json",
            data: {},
            url: $webpath + "/servflow/v1/api/ruleChain/event/" + urlParams.applicationId,
