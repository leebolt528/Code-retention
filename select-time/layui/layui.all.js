                ...
                confirm: function() {
                    ...
                    if (a.range) {
                        if (!n.endDate) {
                            ...
                            function getStamp(timeObject){
                                return timeObject.year+'-'+add0(timeObject.month)+'-'+add0(timeObject.date)+' '+add0(timeObject.hours)+':'+add0(timeObject.seconds)+':'+add0(timeObject.minutes);;
                            }
                            function getTimeGap(startDate,endDate){
                                var startDate = Date.parse(getStamp(startDate));
                                var endDate = Date.parse(getStamp(endDate));
                                if(endDate-startDate > 24*2*60*60*1000){
                                    return "true";
                                }
                            }
                            if(getTimeGap(n.startDate,n.endDate) == "true"){
                                return n.hint("请选择日期范围不大于两天");
                            }
                        }
