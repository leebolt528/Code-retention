            ...
            if (node[nameKey] && node[nameKey].toLowerCase().indexOf(_keywords.toLowerCase())!=-1) {
                if(isHighLight){ //如果高亮，对文字进行高亮处理
                    ...
                    node[nameKey] = node.oldname.replace(rexGlobal, function(originalText){
                        //将所有匹配的子串加上高亮效果
                        var highLightText =
                            '<span style="color: whitesmoke;background-color: #198ccb;">'
                            + originalText
                            +'</span>';
        ...
        if(nodesShow && nodesShow.length>0){
            ...
            if(_keywords.length>0){ 
                $.each(nodesShow, function(n,obj){
                    ...
                    var childrenOfOne = [];
                    childrenOfOne = getAllChildrenNodes(obj, childrenOfOne);
					if(childrenOfOne && childrenOfOne.length>0){
						for(var i=0;i<childrenOfOne.length;i++){
                            zTreeObj.showNode(childrenOfOne[i]); //显示节点
                            zTreeObj.expandNode(childrenOfOne[i],false);
                        }
					}
                });             
